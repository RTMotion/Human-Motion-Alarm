...
// Enter your card-ID here byte Access_UID_Array[2][7] = {{70, 8, 219, 147, 0, 0, 0}, {128, 60,2, 1, 0, 0, 0}}; 
// ######################################### SETUP ##################################################################### 
void setup() {    
pinMode(LED, OUTPUT); 
pinMode(BUZZ, OUTPUT);  
...
Serial.print("RFID receiver initialized."); 
 ...
 { 
 digitalWrite(TILTLED,HIGH); //turn ON the led  
 ...
 */   
 ...
 /*        
 Here we check, if the cards UID is stored in the array, which we have defined at the beginning.         Therefore we need two for-loops. The first one goes trough the stored cards: card 1, card 2, ..., card n.         The second one goes trough the 4 or 7 blocks of the UID.         If every block of the stored card matches with ever block from the present one, then return true.         Attention: If only one card is found with "all-true", the checkUID-function will be terminated, and the true         statement will be returned. All other stored cards won't be checked.        
